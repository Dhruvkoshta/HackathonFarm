...
const { Hand } = pkg;
const Game = function (name, host, host_userId) {
	this.deck = new Deck();
	this.host = host;
		...
		if (this.isStageComplete()) {
			...
			if (numNonFolds == 1) {
				...
				if (this.roundData.bets.length == 1) {
					...
					for (let playerResult of roundResults.playersData) {
	...
	this.calculateMoney = (winnerPot, players) => {
		...
		while (playerInvestments.length > 1) {
			...
			for (let p of playerInvestments) {
				p.invested -= minStack;
			}
	...
	this.evaluateWinners = () => {
		...
		if (Array.isArray(winners)) {
			for (let playerHand of playerArray) {
	...
	this.addPlayer = (playerName, socket, userID) => {
		const player = new Player(playerName, socket, userID, this.debug);
